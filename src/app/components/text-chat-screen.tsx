import { ArrowLeft, Send, BookmarkPlus, AlertCircle, ChevronDown, ChevronUp } from "lucide-react";
...
const generateMockFeedback = (text: string): FeedbackData | undefined => {
  ...
  const feedbacks: FeedbackData[] = [
    {
      grammar: [],
      suggestions: [`더 자연스러운 표현: "${text.replace("I am", "I'm").replace("I want to", "I'd like to")}"`, "상황에 맞는 좋은 표현입니다!"],
    },
    {
      grammar: [],
      suggestions: ["원어민은 보통 이렇게 표현해요: 'That's great!' 또는 'Sounds good!'"],
      slang: ["정중한 영어 표현을 잘 사용했습니다 👍"],
    },
    {
      grammar: [],
      suggestions: ["좋은 문장입니다! 조금 더 구체적으로 말하면 더욱 자연스러워요."],
    },
  ];
...
export function TextChatScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "1", speaker: "ai", text: "Hey! What's up? 😊", timestamp: "10:30" },
    {
      ...
      feedback: {
        grammar: [],
        suggestions: ["더 자연스러운 표현: 'I'm doing well, thanks! How about you?'"],
        slang: ["What's up? = 안녕? / 어떻게 지내?"],
      },
    },
    { id: "3", speaker: "ai", text: "I'm doing great! Wanna grab some coffee later?", timestamp: "10:31" },
  ]);
    ...
    <div className="h-screen flex flex-col bg-[#FAFAFA]">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 py-4 space-y-2 max-w-lg mx-auto">
          ...
          {messages.map((msg) => (
            <div key={msg.id}>
              <div className={`flex gap-2 ${msg.speaker === "user" ? "justify-end" : "justify-start"}`}>
                {msg.speaker === "ai" && (
                  <AvatarDisplay
                ...
                <div className="max-w-[72%]">
                  ...
                  <div className={`flex items-center gap-1.5 mt-1 ${msg.speaker === "user" ? "justify-end" : ""}`}>
              ...
              {/* 인라인 피드백 패널 */}
              {msg.speaker === "user" && msg.feedback && expandedFeedbacks.has(msg.id) && (
                <div className="flex justify-end mt-1">
                  <div className="w-[85%] bg-white rounded-2xl border border-indigo-100 p-3.5 shadow-sm">
                    <div className="flex justify-between items-center mb-2.5">
                      <p className="text-[10px] text-indigo-500 uppercase tracking-wide">AI 피드백</p>
                      <button
                    ...
                    {msg.feedback.grammar && msg.feedback.grammar.length > 0 && (
                      <div className="mb-2 bg-red-50 rounded-xl p-2.5">
                        <div className="flex items-center gap-1 mb-1">
                          <AlertCircle size={11} className="text-red-500" />
                          <span className="text-[10px] text-red-600">문법 오류</span>
                        </div>
                        {msg.feedback.grammar.map((error, idx) => (
                          <p key={idx} className="text-[11px] text-gray-600 pl-1">• {error}</p>
                        ))}
                      </div>
                    )}

                    {msg.feedback.suggestions && msg.feedback.suggestions.length > 0 && (
                      <div className={`${msg.feedback.slang && msg.feedback.slang.length > 0 ? "mb-2" : ""} bg-indigo-50 rounded-xl p-2.5`}>
                        <p className="text-[10px] text-indigo-600 mb-1">💡 추천 표현</p>
                        {msg.feedback.suggestions.map((s, idx) => (
                          <p key={idx} className="text-[11px] text-gray-600">{s}</p>
                        ))}
                      </div>
                    )}
                    ...
                    {msg.feedback.slang && msg.feedback.slang.length > 0 && (
                      <div className="bg-purple-50 rounded-xl p-2.5">
                        <p className="text-[10px] text-purple-600 mb-1">📝 표현 설명</p>
