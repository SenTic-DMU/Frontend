import { ArrowLeft, Plus, Mic, Trash2, ChevronRight, MoreVertical, Pencil, Check, X } from "lucide-react";
...
export function VoiceRoomListScreen() {
  const navigate = useNavigate();
  const [rooms, setRooms] = useState<Room[]>([
    {
      id: "1",
      title: "카페에서 주문하기",
      lastMessage: "Can I get a large latte, please?",
      date: "오늘",
      duration: "12분",
    },
    {
      id: "2",
      title: "비즈니스 미팅",
      lastMessage: "Let's discuss the quarterly results.",
      date: "어제",
      duration: "25분",
    },
    {
      id: "3",
      title: "여행 계획 세우기",
      lastMessage: "Where should we go this summer?",
      date: "3일 전",
      duration: "18분",
    },
  ]);
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-100 px-5 pt-3 pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-gray-900">음성 대화</h1>
              <p className="text-xs text-gray-400">{rooms.length}개의 대화방</p>
            </div>
          </div>
          <button
            onClick={() => navigate("/situation-setup/voice")}
            className="flex items-center gap-1.5 bg-[#4F46E5] text-white px-3.5 py-2 rounded-xl text-sm hover:bg-[#4338CA] active:scale-[0.97] transition-all"
          >
            <Plus size={15} />
            새 대화
          </button>
        ...
        {rooms.length === 0 ? (
          ...
          <div className="px-5 py-4 space-y-2.5">
            {rooms.map((room) => (
              <div
                key={room.id}
                className="w-full bg-white rounded-2xl border border-gray-100 p-4 hover:border-indigo-100 hover:shadow-sm transition-all"
              >
                {editingId === room.id ? (
                  /* 수정 모드 */
                  <div>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        autoFocus
                        className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-all"
                        onKeyDown={(e) => {
                      ...
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center rounded-full bg-indigo-50 hover:bg-indigo-100 transition-colors"
                      >
                  ...
                  <div
                    ...
                    onClick={() => navigate(`/voice-chat/${room.id}`)}
                  >
                    <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center flex-shrink-0">
                      <Mic size={18} className="text-[#4F46E5]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {room.duration && (
                          <span className="text-[10px] text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full ml-2 flex-shrink-0">
                            {room.duration}
                          </span>
                        )}
                      </div>
